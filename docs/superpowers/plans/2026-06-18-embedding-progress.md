...
```bash
cd <repo> && npx vitest run tests/live_indexer.test.ts 2>&1 | tail -10
```
...
```bash
cd <repo> && npx vitest run tests/settings.test.ts 2>&1 | tail -10
```
...
```bash
cd <repo> && npx vitest run 2>&1 | tail -15
```
...
```bash
cd <repo> && git add src/live_indexer.ts src/settings.ts tests/live_indexer.test.ts tests/settings.test.ts && git commit -m "feat(progress): noteCount-Getter + showStatusBar-Setting"
```
...
```bash
cd <repo> && npm run build 2>&1 | tail -20
```
...
```bash
cd <repo> && git add src/main.ts && git commit -m "feat(progress): EmbeddingProgress-State + syncProgress in Plugin"
```
...
```bash
cd <repo> && git add src/settings.ts && git commit -m "feat(progress): Progress-Sektion + Statusleisten-Toggle in Settings"
```
...
```bash
cd <repo> && git add src/main.ts && git commit -m "feat(progress): Statusleisten-Item + setStatusBarVisible"
```
