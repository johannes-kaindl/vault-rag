...
```bash
git -C <repo> add src/frontmatter.ts tests/frontmatter.test.ts
...
```bash
git -C <repo> add src/smart_apply.ts tests/smart_apply.test.ts
git -C <repo> commit -m "$(cat <<'EOF'
...
Relevant file paths: `<repo>/tests/__mocks__/obsidian.ts` (modified), pattern source `<repo>/src/note_picker.ts`, house-style reference `<repo>/tests/chat_view.test.ts`.
...
- [ ] **Step 16: Commit (stage only the two touched files)**
  ```bash
  git -C <repo> add src/settings.ts tests/settings.test.ts
  git -C <repo> commit -m "feat(settings): Smart-Apply-Sektion (smartApplyEnabled/templateDir/smartApplyTemperature) + Backward-Compat-Defaults
