...
Run: `cd /Users/Shared/code/vault-rag && npx vitest run tests/chat_client.test.ts 2>&1 | tail -8`
...
```bash
cd /Users/Shared/code/vault-rag && git add src/chat_client.ts tests/chat_client.test.ts && git commit -m "feat(chat): ChatClient + parseSSE (OpenAI-kompatibles Streaming)
...
Run: `cd /Users/Shared/code/vault-rag && npx vitest run tests/context_source.test.ts 2>&1 | tail -8`
Expected: FAIL — Modul fehlt.
...
```bash
cd /Users/Shared/code/vault-rag && git add src/context_source.ts tests/context_source.test.ts && git commit -m "feat(chat): assembleContext — 3 Kontext-Strategien (auto-rag/aktiv/gewählt)
...
Run: `cd /Users/Shared/code/vault-rag && npx vitest run tests/chat_session.test.ts 2>&1 | tail -8`
Expected: FAIL — Modul fehlt.
...
```bash
cd /Users/Shared/code/vault-rag && git add src/chat_session.ts tests/chat_session.test.ts && git commit -m "feat(chat): ChatSession — Multi-Turn-Orchestrierung + Fehlerpfade
...
Run: `cd /Users/Shared/code/vault-rag && npx vitest run tests/settings.test.ts 2>&1 | tail -8`
...
Run: `cd /Users/Shared/code/vault-rag && npx vitest run 2>&1 | tail -6`
Expected: PASS (alle).

- [ ] **Step 6: Commit**

```bash
cd /Users/Shared/code/vault-rag && git add src/settings.ts tests/settings.test.ts && git commit -m "feat(chat): Chat-Settings (Endpoint, Modell, chatK, Kontext-Budget)
...
Run: `cd /Users/Shared/code/vault-rag && npx vitest run tests/chat_view.test.ts 2>&1 | tail -8`
Expected: FAIL — Modul fehlt.
...
```bash
cd /Users/Shared/code/vault-rag && git add src/chat_view.ts tests/chat_view.test.ts && git commit -m "feat(chat): ChatView (Sidebar, Modus-Umschalter, Quellen-Chips, Stop)
...
Run:
```bash
cd /Users/Shared/code/vault-rag && npx tsc --noEmit 2>&1 | tail -5; echo "tsc=$?"
...
```bash
cd /Users/Shared/code/vault-rag && git add src/main.ts styles.css && git commit -m "feat(chat): Vault-Chat verdrahten (View, Ribbon, Command, assembleChatContext)
