...
Run: `cd <repo> && npx vitest run tests/embed_vector.test.ts 2>&1 | tail -10`
...
```bash
cd <repo> && git add src/embed_vector.ts tests/embed_vector.test.ts && git commit -m "feat(search): toIndexVector — geteilte Embedding→Vektor-Transformation
...
Run: `cd <repo> && npx vitest run tests/live_indexer.test.ts 2>&1 | tail -10`
...
```bash
cd <repo> && git add src/live_indexer.ts && git commit -m "refactor(indexer): update() nutzt toIndexVector (DRY)
...
Run: `cd <repo> && npx vitest run tests/retriever.test.ts 2>&1 | tail -10`
...
```bash
cd <repo> && git add src/retriever.ts tests/retriever.test.ts && git commit -m "feat(search): Retriever.search(queryVec) + privates rank()
...
Run: `cd <repo> && npx vitest run tests/view.test.ts 2>&1 | tail -10`
...
```bash
cd <repo> && git add src/view.ts tests/view.test.ts && git commit -m "refactor(view): renderHits extrahieren (geteilt mit Such-Panel)
...
Run: `cd <repo> && npx vitest run tests/search_view.test.ts 2>&1 | tail -10`
...
```bash
cd <repo> && git add src/search_view.ts tests/search_view.test.ts && git commit -m "feat(search): SemanticSearchView (Sidebar-Panel, Zustände)
...
Run:
```bash
cd <repo> && npm run build 2>&1 | tail -8 && npx vitest run 2>&1 | tail -12
```
...
```bash
cd <repo> && git add src/main.ts styles.css && git commit -m "feat(search): Such-Panel verdrahten (View, Ribbon, Command, runSearch)
