...
5. Select a block of text, then run **Abschnitt umformatieren** from the command palette or the editor right-click menu, and pick a transform. Mechanical ones apply immediately; LLM ones open a streamed preview to review before applying. *Known v1 limitation:* the selection range is captured when you invoke the command — if you edit the note while the LLM preview modal is still open, the captured range can go stale and the replacement may land in the wrong place.
